...
const EditCompetes = ({ item }) => {
    ...
    const update = async e => {
        ...
        try {
            console.log("helooo")
            const body = { event_id, player_id, year, };
            console.log(JSON.stringify(body))
            const response = await fetch(`http://localhost:5000/competes/${evid}/${pid}/${yearid}`, {
                method: "PUT",
