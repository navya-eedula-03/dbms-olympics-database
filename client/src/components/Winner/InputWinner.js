...
const InputWinner = () => {
    ...
    let competesList = competes.length > 0
        
        && competes.map((item, i) => {

            if(item)
            return (
                <option key={item.player_id} value={item.competes_id}>{item.year}</option>
            )
        }, this);
    ...
    return (
        <Fragment>
            <h1 className='text-center mt-5'>WINNER</h1>
            ...
            <form className="form-inline" onSubmit={onSubmitForm}>
                ...
                <div className="form-group">
                    ...
                    <select class="form-control" onChange={e => setMedal(e.target.value)}>
